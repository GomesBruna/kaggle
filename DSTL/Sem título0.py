...
train_wkt = pd.read_csv('/home/gomesbruna/Documentos/dataScience/Kaggle/DSTL/input/train_wkt_v4.csv')

## Load sample submission
sample_submission = pd.read_csv('/home/gomesbruna/Documentos/dataScience/Kaggle/DSTL/input/sample_submission.csv')

## Load grid size
grid_sizes = pd.read_csv('/home/gomesbruna/Documentos/dataScience/Kaggle/DSTL/input/grid_sizes.csv')
...
def image_to_train(image_id, class_type, image_type ='3'):
    ...
    if image_type =='3':
        image = tiff.imread('/home/gomesbruna/Documentos/dataScience/Kaggle/DSTL/input/three_band/{}.tif'.format(image_id)).transpose([1, 2, 0])
        X = image.reshape(-1, 3).astype(np.float32) 
        image_size = image.shape[:2]
    else:
        X_P = tiff.imread('/home/gomesbruna/Documentos/dataScience/Kaggle/DSTL/input/sixteen_band/{}_P.tif'.format(image_id))
        nomRow = X_P.shape[0]
        nomCol = X_P.shape[1]
        
#        Tracer()()
        imageMP = func_feature_scale('/home/gomesbruna/Documentos/dataScience/Kaggle/DSTL/input/sixteen_band/{}_M.tif'.format(image_id), nomRow, nomCol)
        X_M = imageMP.reshape(-1, 8).astype(np.float32) 

        imageAP = func_feature_scale('/home/gomesbruna/Documentos/dataScience/Kaggle/DSTL/input/sixteen_band/{}_A.tif'.format(image_id), nomRow, nomCol)
